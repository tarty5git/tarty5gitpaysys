package com.payment.platform.provider.stripe.router;
...
public class StripePaymentRouter implements PaymentRouter {

    private final List<PaymentHandler> handlers;
    private final AtomicInteger roundRobinIndex = new AtomicInteger(0);
    private final boolean enabled;

    public StripePaymentRouter(List<PaymentHandler> initialHandlers) {
        this(initialHandlers, true);
    }

    public StripePaymentRouter(List<PaymentHandler> initialHandlers, boolean enabled) {
    ...
    private PaymentHandler selectAvailableHandler() {
        ...
        if (availableHandlers.isEmpty()) {
            throw new PaymentCapacityExceededException("Stripe capacity exceeded: No available payment handlers in pool.");
        }
