package com.payment.platform.provider.paypal.router;
...
public class PaypalPaymentRouter implements PaymentRouter {

    private final List<PaymentHandler> handlers;
    private final AtomicInteger roundRobinIndex = new AtomicInteger(0);
    private final boolean enabled;

    public PaypalPaymentRouter(List<PaymentHandler> initialHandlers) {
        this(initialHandlers, true);
    }

    public PaypalPaymentRouter(List<PaymentHandler> initialHandlers, boolean enabled) {
    ...
    private PaymentHandler selectAvailableHandler() {
        ...
        if (availableHandlers.isEmpty()) {
            throw new PaymentCapacityExceededException("PayPal capacity exceeded: No available payment handlers in pool.");
        }
